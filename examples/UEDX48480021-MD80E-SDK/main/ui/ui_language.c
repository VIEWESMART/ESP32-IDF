...
void ui_language_init(lv_obj_t *parent)
{
    ...
    lv_obj_set_style_text_color(labelinfo_top, lv_color_white(), 0);
    lv_obj_set_style_text_font(labelinfo_top, &HelveticaNeue_Regular_40, 0);
    lv_label_set_text(labelinfo_top, "Please select a");
    lv_obj_align(labelinfo_top, LV_ALIGN_TOP_MID, 0, 115);

    lv_obj_t *labelinfo_bottom = lv_label_create(page);
    lv_obj_set_style_text_color(labelinfo_bottom, lv_color_white(), 0);
    lv_obj_set_style_text_font(labelinfo_bottom, &HelveticaNeue_Regular_40, 0);
    lv_label_set_text(labelinfo_bottom, "language");
    lv_obj_align(labelinfo_bottom, LV_ALIGN_TOP_MID, 0, 150);

    imgbtn_lang_EN = lv_img_create(page);
    lv_obj_align(imgbtn_lang_EN, LV_ALIGN_CENTER, -80, 50);
    lv_img_set_src(imgbtn_lang_EN, &language_select);
    lv_obj_add_state(imgbtn_lang_EN, LV_STATE_CHECKED);

    label_EN = lv_label_create(imgbtn_lang_EN);
    lv_obj_set_style_text_font(label_EN, &HelveticaNeue_Regular_40, 0);
    ...
    lv_obj_align(imgbtn_lang_CN, LV_ALIGN_CENTER, 80, 50);
    lv_img_set_src(imgbtn_lang_CN, &language_unselect);
    lv_obj_clear_state(imgbtn_lang_CN, LV_STATE_CHECKED);

    label_CN = lv_label_create(imgbtn_lang_CN);
    lv_obj_set_style_text_font(label_CN, &font_SourceHanSansCN_38, 0);
