...
typedef struct {
    const char *name_CN;
    const char *name_EN;
    const lv_img_dsc_t *icon_checked;
    const lv_img_dsc_t *icon_unchecked;
...
{
    return ui_get_num_offset(app_selected, APP_NUM, offset);
}
...
{
    const int ox = 0, oy = 0;
    *x = ox + ((lv_trigo_sin(theta) * 60) >> LV_TRIGO_SHIFT);
    *y = oy + ((lv_trigo_cos(theta) * 60) >> LV_TRIGO_SHIFT);
}
...
static void menu_event_cb(lv_event_t *e)
{
    ...
    if (LV_EVENT_FOCUSED == code) {
        ...
        if (is_time_out(&time_100ms)) {
            int8_t last_index = app_selected;
            if (LV_KEY_RIGHT == key) {
                app_selected = get_app_index(-1);
            } else if (LV_KEY_LEFT == key) {
                app_selected = get_app_index(1);
            }
            if ((factory_Enter < 6) && (app_selected == 2)) {
                factory_Enter = 7;
                ESP_LOGI(TAG, "Invalid Enter factory");
            }

            if ((factory_Enter < 6) && (++factory_Enter == 6) && (app_selected == 0)) {
            ...
            for (int i = 0; i < APP_NUM; i++) {
                obj_set_to_hightlight(icons[i], i == app_selected);
            }
            lv_obj_swap(icons[last_index], icons[get_app_index(0)]);
            lv_img_set_src(icons[last_index], menu[last_index].icon_unchecked);
            lv_img_set_src(icons[get_app_index(0)], menu[get_app_index(0)].icon_checked);
...
void ui_menu_init(lv_obj_t *parent)
{
    ...
    lv_obj_set_style_border_width(page, 10, 0);
    lv_obj_set_style_border_color(page, menu[get_app_index(0)].theme_color, 0);
    ...
    for (int i = APP_NUM - 1; i >= 0; i--) {
        ...
        if (i == app_selected) {
            lv_img_set_src(icons[i], menu[i].icon_checked);
        } else {
            lv_img_set_src(icons[i], menu[i].icon_unchecked);
        }
    ...
    if (0 != app_selected) {
        lv_obj_swap(icons[0], icons[app_selected]);
    }

    label_name = lv_label_create(page);
    sys_param_t *param = settings_get_parameter();
    if (LANGUAGE_CN == param->language) {
        lv_obj_set_style_text_font(label_name, &font_SourceHanSansCN_38, 0);
        lv_label_set_text(label_name, menu[app_selected].name_CN);
    } else {
        lv_obj_set_style_text_font(label_name, &HelveticaNeue_Regular_40, 0);
        lv_label_set_text(label_name, menu[app_selected].name_EN);
    }
    lv_obj_set_style_text_color(label_name, lv_color_hex(COLOUR_WHITE), 0);
    lv_obj_set_width(label_name, 250);  /*Set smaller width to make the lines wrap*/
    ...
    lv_obj_set_size(tips_btn, 400, 40);
    ...
    if (LANGUAGE_CN == param->language) {
        lv_obj_set_style_text_font(tips_label, &font_SourceHanSansCN_38, 0);
        lv_label_set_text(tips_label, "已恢复出厂");
    } else {
        lv_obj_set_style_text_font(tips_label, &HelveticaNeue_Regular_40, 0);
...
static bool main_layer_enter_cb(void *layer)
{
    ...
    if (NULL == create_layer->lv_obj_layer) {
        ...
        lv_obj_set_style_bg_color(create_layer->lv_obj_layer, lv_color_hex(COLOUR_BLACK), 0);
