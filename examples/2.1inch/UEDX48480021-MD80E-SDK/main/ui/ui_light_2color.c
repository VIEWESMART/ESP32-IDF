...
void ui_light_2color_init(lv_obj_t *parent)
{
    ...
    lv_obj_set_style_border_color(page, lv_color_make(0xFC, 0xAE, 0x49), 0);
    ...
    lv_obj_set_style_text_color(label_pwm_set, lv_color_white(), 0);
    lv_obj_set_style_text_font(label_pwm_set, &HelveticaNeue_Regular_48, 0);
    if (light_set_conf.light_pwm) {
        lv_label_set_text_fmt(label_pwm_set, "%d%%", light_set_conf.light_pwm);
    } else {
        lv_label_set_text(label_pwm_set, "--");
    }
    lv_obj_align(label_pwm_set, LV_ALIGN_CENTER, 0, 125);
    ...
    lv_obj_align(img_light_pwm_75, LV_ALIGN_TOP_MID, 0, 70);
    ...
    lv_obj_align(img_light_pwm_100, LV_ALIGN_TOP_MID, 0, 70);
    ...
    if (is_time_out(&time_20ms)) {
        ...
        if ((light_set_conf.light_pwm ^ light_xor.light_pwm) || (light_set_conf.light_cck ^ light_xor.light_cck)) {
            ...
            case 25:
                ...
                if (LIGHT_CCK_COOL == cck_set) {
                    lv_obj_set_style_border_color(page, lv_color_make(0xC9, 0xC2, 0xD1), 0);
                } else {
                    lv_obj_set_style_border_color(page, lv_color_make(0xFC, 0xAE, 0x49), 0);
                }
                break;
