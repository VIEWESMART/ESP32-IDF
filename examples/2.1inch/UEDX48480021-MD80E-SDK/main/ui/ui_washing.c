...
#define     FUNC_NUM                        3
#define     WASH_NAME_Y_OFFSET              130
#define     WASH_NAME_X_MID_OFFSET          100
...
static void menu_position_reset()
{
    ...
    for (int i = 0; i < FUNC_NUM; i++) {
        if (i == item_top) {
            abs_t = WASH_NAME_Y_OFFSET;
            x_axis = 20;
            y_axis = (0 - WASH_NAME_Y_OFFSET);
        } else if (i == item_central) {
            abs_t = 0;
            x_axis = WASH_NAME_X_MID_OFFSET;
            y_axis = (0);
        ...
        zoom_set = 256 + (256 * 3)/(1 + LV_ABS(abs_t))/10;//256 * (1 + 0.3)
        lv_img_set_zoom(img_funcs[i], zoom_set);
        lv_obj_set_style_img_recolor_opa(img_funcs[i], LV_OPA_COVER, 0);
        lv_obj_set_style_img_recolor(img_funcs[i], lv_color_hsv_to_rgb(200, (WASH_NAME_Y_OFFSET + 1 - abs_t) * 60 / WASH_NAME_Y_OFFSET, 100), 0);
...
static void func_anim_cb(void *args, int32_t v)
{
    ...
    for (int i = 0; i < FUNC_NUM; i++) {
        ...
        if (i == item_central) {
            ...
            if (v > (WASH_NAME_Y_OFFSET/2)) {  /*65 - 130*/
                x_axis = 0 + v / 4;
                y_axis = dir * (120 - v / 2);
            } else { /*0 - 65*/
                x_axis = cycle_init_x_axis[i] - v / 2;
        ...
        if (abs_t <= (WASH_NAME_Y_OFFSET/2)) {
            uint16_t zoom_set = 256 + (256 * 3)/(1 + LV_ABS(abs_t))/10;//256 * (1 + 0.3)
            lv_img_set_zoom(img_funcs[i], zoom_set);
...
static void washing_event_cb(lv_event_t *e)
{
    ...
    if (LV_EVENT_FOCUSED == code) {
        ...
        if (direct_changed) {
            lv_anim_t a1;
            lv_anim_init(&a1);
            lv_anim_set_var(&a1, (void *)direct_changed);
            lv_anim_set_delay(&a1, 0);
            lv_anim_set_values(&a1, 0, WASH_NAME_Y_OFFSET - LV_ABS(cycle_init_y_axis[item_central]));
            ...
            lv_anim_set_user_data(&a1, (void *)direct_changed);
...
void ui_washing_init(lv_obj_t *parent)
{
    ...
    lv_obj_set_style_border_width(page_background, 10, 0);
    ...
    lv_obj_set_style_text_color(label_leftTimeH, lv_color_hex(COLOUR_WHITE), 0);
    lv_obj_set_style_text_font(label_leftTimeH, &HelveticaNeue_Regular_80, 0);
    lv_label_set_text(label_leftTimeH, "12");
    lv_obj_align(label_leftTimeH, LV_ALIGN_CENTER, -60, -40);

    label_leftTimeL = lv_label_create(page_run);
    lv_obj_set_style_text_color(label_leftTimeL, lv_color_hex(COLOUR_WHITE), 0);
    lv_obj_set_style_text_font(label_leftTimeL, &HelveticaNeue_Regular_80, 0);
    lv_label_set_text(label_leftTimeL, "04");
    lv_obj_align(label_leftTimeL, LV_ALIGN_CENTER, 60, -40);

    label_leftTime_unit = lv_label_create(page_run);
    lv_obj_set_style_text_color(label_leftTime_unit, lv_color_hex(COLOUR_WHITE), 0);
    lv_obj_set_style_text_font(label_leftTime_unit, &HelveticaNeue_Regular_80, 0);
    lv_label_set_text(label_leftTime_unit, ":");
    lv_obj_align(label_leftTime_unit, LV_ALIGN_CENTER, -8, -43);
    ...
    if (LANGUAGE_CN == param->language) {
        lv_obj_set_style_text_font(label_info, &SourceHanSansCN_Normal30, 0);
    ...
    lv_obj_align(img_run_wave1, LV_ALIGN_BOTTOM_MID, -15, 5);
    lv_img_set_zoom(img_run_wave1, 600);
    img_run_wave2 = lv_img_create(page_run);
    lv_img_set_src(img_run_wave2, &img_washing_wave2);
    lv_obj_align(img_run_wave2, LV_ALIGN_BOTTOM_MID, 20, 5);
    lv_img_set_zoom(img_run_wave2, 600);
    ...
    lv_img_set_src(img_bg_wash, &img_washing_cut_bg);
    lv_obj_align(img_bg_wash, LV_ALIGN_LEFT_MID, 20, 0);

    img_wave1 = lv_img_create(img_bg_wash);
    lv_img_set_src(img_wave1, &img_washing_wave1);
    lv_img_set_zoom(img_wave1, 300);
    ...
    lv_obj_align(img_anmi_shirt, LV_ALIGN_TOP_MID, 0, 49);

    img_anmi_underwear1 = lv_img_create(img_bg_wash);
    lv_img_set_src(img_anmi_underwear1, &wash_underwear1);
    lv_obj_align(img_anmi_underwear1, LV_ALIGN_TOP_MID, 0, 31);
    img_anmi_underwear2 = lv_img_create(img_bg_wash);
    lv_img_set_src(img_anmi_underwear2, &wash_underwear2);
    lv_obj_align(img_anmi_underwear2, LV_ALIGN_TOP_MID, 0, 89);

    label_wash_time = lv_label_create(page_standby);
    lv_obj_set_style_text_font(label_wash_time, &SourceHanSansCN_Normal30, 0);
    lv_obj_set_style_text_color(label_wash_time, lv_color_hex(COLOUR_GREY_BF), 0);
    lv_label_set_text_fmt(label_wash_time, "- %02d min -", wash_cycle[item_central].wash_time);
    ...
    lv_obj_align(label_wash_time, LV_ALIGN_CENTER, 100, 50);
    ...
    for (size_t i = 0; i < FUNC_NUM; i++) {
        ...
        x = WASH_NAME_X_MID_OFFSET;
        y = (i - 1) * WASH_NAME_Y_OFFSET;
